...
NAME = os.getenv("IMAGE_NAME", "nautobot-plugin-chatops-ansible")
PWD = os.getcwd()

COMPOSE_FILE = "development/docker-compose.yml"
BUILD_NAME = "nautobot_plugin_chatops_ansible"
...
def destroy(context, nautobot_ver=NAUTOBOT_VER, python_ver=PYTHON_VER):
    ...
    context.run(
        f"docker volume rm -f {BUILD_NAME}_pgdata_nautobot_plugin_chatops_ansible",
        env={"NAUTOBOT_VER": nautobot_ver, "PYTHON_VER": python_ver},
    )
...
def unittest(context, nautobot_ver=NAUTOBOT_VER, python_ver=PYTHON_VER):
    ...
    context.run(
        f'{docker} sh -c "nautobot-server test nautobot_plugin_chatops_ansible"',
        env={"NAUTOBOT_VER": nautobot_ver, "PYTHON_VER": python_ver},
        pty=True,
    )
