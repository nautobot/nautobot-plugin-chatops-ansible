...
NAME = os.getenv("IMAGE_NAME", "ntc-netbox-plugin-nautobot-ansible")
PWD = os.getcwd()

COMPOSE_FILE = "development/docker-compose.yml"
BUILD_NAME = "netbox_nautobot_ansible"
...
@task
def generate_packages(context, netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    Args:
        ...
        python_ver (str): Will use the Python version docker image to build from
    """
    container_name = f"{BUILD_NAME}_netbox_package"
    context.run(
        f"docker rm {container_name} || true", env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver}, pty=True,
    )
    context.run(
        f"docker-compose  -f {COMPOSE_FILE} -p {BUILD_NAME} run --name {container_name} -w /source netbox poetry build",
        env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
    )
    context.run(
...
def destroy(context, netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    context.run(
        f"docker volume rm -f {BUILD_NAME}_pgdata_netbox_nautobot_ansible",
        env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
    )
...
def unittest(context, netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    context.run(
        f'{docker} sh -c "python manage.py test netbox_nautobot_ansible"',
        env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        pty=True,
    )
